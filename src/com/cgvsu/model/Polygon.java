...
public class Polygon extends JPanel {
    ...
    public static boolean equals(final ArrayList<Polygon> p1, final ArrayList<Polygon> p2) {
        if (p1.size() != p2.size()) {
            return false;
        }
        for (int i = 0; i < p1.size(); i++) {
            if (!p1.get(i).getVertexIndices().equals(p2.get(i).getVertexIndices())
                    || !p1.get(i).getNormalIndices().equals(p2.get(i).getNormalIndices())
                    || !p1.get(i).getTextureVertexIndices().equals(p2.get(i).getTextureVertexIndices())) {
                return false;
            }
        }
        return true;
